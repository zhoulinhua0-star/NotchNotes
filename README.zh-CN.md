...
| --- | --- | --- |
| 只保存路径引用，原文件始终留在原位 | 管理窗口和 Space 时自动隐藏 Shelf、暂停 Hover | 单击菜单栏图标即可防止空闲睡眠，刘海旁显示已开启时长；合盖或选择“睡眠”后自动关闭 |
...
- 如果引用的文件或文件夹被移动、移到废纸篓或删除，Shelf 会在下次展开、变为活跃或你尝试拖出时把它变暗并显示警告；不会自动寻找它的新位置。
- 暂存记录保存在本机，最多保留 100 项。

### Click / Hover — 在需要时出现

从 Shelf 右上角的齿轮或右键菜单栏图标选择 `Open Shelf With`：
...
单击菜单栏的文件托盘图标，或点击 Shelf 右上角的咖啡杯即可切换。NotchNotes 会在后台运行：

```bash
/usr/bin/caffeinate -di -w <NotchNotes PID>
```

- **菜单栏图标**：Keep Awake 关闭时，文件托盘图标是轮廓样式；开启后变为填充样式。每次切换时，图标会短暂变成咖啡杯：开启时弹跳出现，关闭时向下淡出。Keep Awake 停止后图标立即恢复为轮廓样式，后台 `caffeinate` 进程意外退出时也一样。
- **刘海旁的计时**：Shelf 顶部刘海两侧的窄带，每开启 10 分钟点亮一格，并显示已开启的时长，例如 `1h 24m`。每次展开 Shelf 时，格子会从刘海向外依次亮起。
- **右键菜单**：右键或按住 Control 单击菜单栏图标，可以选择 `Keep Mac Awake`、`Open Shelf With` 和 `Quit NotchNotes`。添加文件请在 Shelf 里点 `Add Files` 或按 <kbd>⌘</kbd> + <kbd>O</kbd>。

如果在系统设置中开启了「减弱动态效果」，图标和计时会直接切换，不播放动画。

| 操作 | Keep Awake 状态 |
| --- | --- |
| 单击菜单栏图标或咖啡杯开启 | 防止显示器休眠和 Mac 因空闲进入睡眠 |
...
- 基于 `/usr/bin/caffeinate` 的基础防休眠：单击菜单栏图标切换，在 Shelf 里显示已开启时长，并在系统睡眠前自动关闭。
...
- **Swift + AppKit**：菜单栏应用（单击切换的状态栏图标与 SF Symbols 动效）、浮层窗口、屏幕定位、拖放与顶部指针触发；触发区域限制在菜单栏范围内，仅在文件拖拽期间向下扩展，并使用系统窗口可见性隔离 Mission Control。
- **SwiftUI**：File Shelf、选择状态、设置菜单、咖啡杯控制，以及刘海旁的 Keep Awake 计时。
