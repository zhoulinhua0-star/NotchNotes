...
| --- | --- | --- |
| 只保存路径引用，原文件始终留在原位 | 管理窗口和 Space 时自动隐藏 Shelf、暂停 Hover | 防止空闲睡眠，启用时填充菜单栏图标；合盖或选择“睡眠”后自动关闭 |
...
Keep Awake 关闭时，菜单栏的文件托盘图标保持轮廓样式；启用后图标会变为填充样式，无需打开菜单即可确认状态。Keep Awake 停止后，图标会立即恢复为轮廓样式；后台 `caffeinate` 进程意外退出时也一样。
